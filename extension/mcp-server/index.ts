...
const GetCurrentSprintSchema = z.object({
  projectId: z.string().optional(),
  teamId: z.string().optional(),
});
...
const GetSprintWorkItemsSchema = z.object({
  sprintId: z.string().optional(),
  iterationPath: z.string().optional(),
...
const AssignWorkItemsToSprintSchema = z.object({
  workItemIds: z.array(z.number().int().positive()),
  sprintId: z.string().optional(),
  iterationPath: z.string().optional(),
});

const RemoveWorkItemsFromSprintSchema = z.object({
  workItemIds: z.array(z.number().int().positive()),
  sprintId: z.string().optional(),
});
...
const CreateSprintSchema = z.object({
  ...
  startDate: z.string().min(1, 'Start date is required (ISO format: YYYY-MM-DD)'),
  endDate: z.string().min(1, 'End date is required (ISO format: YYYY-MM-DD)'),
...
const AddSprintToTeamSchema = z.object({
  sprintId: z.string().min(1, 'Sprint ID is required'),
  teamName: z.string().optional(),
  projectId: z.string().optional(),
});

const RemoveSprintFromTeamSchema = z.object({
  sprintId: z.string().min(1, 'Sprint ID is required'),
  teamName: z.string().optional(),
  projectId: z.string().optional(),
});
...
class AzureDevOpsApiClient {
  ...
  async getCurrentSprint(projectId?: string, teamId?: string): Promise<any> {
  ...
  async getSprint(sprintId?: string, iterationPath?: string): Promise<any> {
  ...
  async getSprintWorkItems(sprintId?: string, iterationPath?: string, workItemTypes?: string[], includeCompleted: boolean = true): Promise<any[]> {
    ...
    try {
      ...
      const detailsData = await detailsResponse.json();
      const workItems = detailsData.value.map((item: any) => this.mapAzureWorkItemToWorkItem(item));
  ...
  async assignWorkItemsToSprint(workItemIds: number[], sprintId?: string, iterationPath?: string): Promise<any[]> {
  ...
  async removeWorkItemsFromSprint(workItemIds: number[], sprintId?: string): Promise<any[]> {
  ...
  // Sprint CRUD Methods
  async createSprint(name: string, startDate: string, endDate: string, teamName: string, projectId?: string, description?: string): Promise<any> {
    const projectToUse = projectId || this.projectName;
  ...
  async updateSprint(sprintId: string, name?: string, startDate?: string, endDate?: string, description?: string): Promise<any> {
  ...
  async deleteSprint(sprintId: string): Promise<any> {
  ...
  async addSprintToTeam(sprintId: string, teamName?: string, projectId?: string): Promise<any> {
    const projectToUse = projectId || this.projectName;
  ...
  async removeSprintFromTeam(sprintId: string, teamName?: string, projectId?: string): Promise<any> {
    const projectToUse = projectId || this.projectName;
    const teamToUse = teamName || `${projectToUse} Team`;
...
export class AzureDevOpsCoreServer {
  ...
  private async getCurrentSprint(args: any) {
    this.ensureInitialized();
  ...
  private async getSprint(args: any) {
    this.ensureInitialized();
  ...
  private async getSprintWorkItems(args: any) {
    this.ensureInitialized();
    ...
    const workItemList = workItems.map(wi => 
      `- #${wi.id}: ${wi.title} (${wi.type}, ${wi.state})`
    ).join('\n');
  ...
  private async assignWorkItemsToSprint(args: any) {
    this.ensureInitialized();
    ...
    const resultText = results.map(r => 
      `- Work Item #${r.workItemId}: ${r.success ? '✅' : '❌'} ${r.message}`
    ).join('\n');
  ...
  private async removeWorkItemsFromSprint(args: any) {
    this.ensureInitialized();
  ...
  private async deleteSprint(args: any) {
    this.ensureInitialized();
  ...
  private async addSprintToTeam(args: any) {
    this.ensureInitialized();
  ...
  private async removeSprintFromTeam(args: any) {
    this.ensureInitialized();
