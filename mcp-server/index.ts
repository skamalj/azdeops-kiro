...
class AzureDevOpsApiClient {
  ...
  async createTestCase(fields: any, testPlanId?: number) {
    ...
    if (!r.ok) throw new Error("Failed to create test case");
    return this.mapAzureWorkItemToWorkItem(await r.json());
  }

  async createTestPlan(fields: any) {
    const testPlanData = {
      name: fields.name,
    ...
    if (!r.ok) throw new Error("Failed to create test plan");
    return await r.json();
  }

  async getTestCases(projectId?: string, testPlanId?: number, maxResults = 50) {
    ...
    const detailRes = await fetch(
      `${this.getBaseUrl()}/_apis/wit/workitems?ids=${ids
        .slice(0, maxResults)
        .join(",")}&$expand=all&api-version=7.0`,
      { headers: { Authorization: this.getAuthHeader() } }
    );

    const final = await detailRes.json();
    return final.value.map((item: any) => this.mapAzureWorkItemToWorkItem(item));
  }
  ...
  async getTestPlans(projectId?: string) {
    ...
    if (!r.ok) throw new Error("Failed to get test plans");
    const data = await r.json();
    return data.value;
  }

  async executeTestCase(testCaseId: number, result: any) {
    const patchDocument = [
      {
        op: "replace",
    ...
    if (!r.ok) throw new Error("Failed to execute test case");
    return this.mapAzureWorkItemToWorkItem(await r.json());
  }
  ...
  private mapAzureProjectToProject(azure: any) {
    return {
      id: azure.id,
      name: azure.name,
  ...
  private buildTestCasePatchDocument(fields: any, testPlanId?: number) {
    const patchDocument = [
      { op: "add", path: "/fields/System.Title", value: fields.title },
  ...
  private buildTestStepsXml(steps: any[]) {
  ...
  private mapPriorityToNumber(priority: string) {
    switch (priority) {
  ...
  async batchCreateUserStories(stories: any[], batchSize: number) {
    const results = [];
    const errors = [];
  ...
  async batchCreateTasks(tasks: any[], batchSize: number) {
    const results = [];
    const errors = [];
  ...
  async batchCreateTestCases(testCases: any[], batchSize: number) {
    const results = [];
    const errors = [];
  ...
  async batchCreateTestPlans(testPlans: any[], batchSize: number) {
    const results = [];
    const errors = [];
...
export class AzureDevOpsCoreServer {
  ...
  private async getProjects(args: any) {
    ...
    return {
      content: [
        {
          type: "text",
          text: projects.map((p: any) => `${p.name} (${p.id}) - ${p.state}`).join("\n"),
        },
      ],
    };
  }
  ...
  private async getTestCases(args: any) {
    ...
    const testCases = await this.apiClient.getTestCases(
      parsed.projectId,
      parsed.testPlanId,
      parsed.maxResults
    );
      ...
      content: [
        {
          ...
          text: testCases.length > 0 
            ? testCases.map((tc: any) => `#${tc.id}: ${tc.title} (${tc.state})`).join("\n")
            : "No test cases found",
        },
      ],
    };
      ...
      content: [
        {
          ...
          text: testPlans.length > 0
            ? testPlans.map((tp: any) => `#${tp.id}: ${tp.name} (${tp.state})`).join("\n")
            : "No test plans found",
        },
      ],
    };
  ...
  private async batchCreateUserStories(args: any) {
    ...
    if (result.successful > 0) {
      ...
      result.results.slice(0, 10).forEach((item: any) => {
    ...
    if (result.failed > 0) {
      responseText += `\n⚠️ Failed Stories:\n`;
      result.errors.slice(0, 5).forEach((item: any) => {
